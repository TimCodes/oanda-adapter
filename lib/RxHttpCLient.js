...
class OandaAdaptorRx {
    ...
    getOHLCSteam(symbol, timeFrame ){
        ...
        // date shoudl be set accoaurding to time unit 
        let date = new Date();
        // set time for 1 min from now on the minute
        date.setMinutes(date.getMinutes() + 1)

        return Rx.Observable.timer(date, interval)
        .flatMap ( d=> od.getOHLCByCount('EUR_USD', 1, 'M1') )
        .map(d => JSON.parse(d.body).candles[0] )
...
function getMinutesRoundedDown(mintueValue){
...
console.log('round down ----', getMinutesRoundedDown(10))

let date = new Date(); 
let mins = getMinutesRoundedDown(1) + 1;
// set time for 1 min from now on the minute
date.setMinutes(date.getMinutes() + 1)

Rx.Observable.timer(date, 2000)
.flatMap ( d=> od.getOHLCByCount('EUR_USD', 1, 'M1') )
.map(d => JSON.parse(d.body).candles[0] )
.subscribe(console.log)
...
od.getOHLCByCount('EUR_USD', 1, 'M15')
.subscribe(
       (data) => {
      //  console.log(data)
        if (data.response.statusCode === 200) {
    ...
    function (err) {
        console.log('obersver cauth the error, Error: ' + err);
    },
    function () {
        console.log('Completed');
    })
